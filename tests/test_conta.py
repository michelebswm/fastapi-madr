...
def test_put_conta_ok(client, user, token):
    response = client.put(
        f'/conta/{user.id}',
...
def test_put_conta_unauthorized(client, token, user):
    response = client.put(
        f'/conta/{user.id + 1}',
        headers={'Authorization': f'Bearer {token}'},
        json={'username': 'newusername', 'email': 'newemail@test.com', 'senha': '1234'},
...
def test_delete_ok(client, token, user):
...
def test_delete_unauthorized(client, token, user):
